...
An alternative visualization to try to understand temporal density patters:

```{r}
crime_v2 |> 
  filter(year(start_date)==2022) |>
  mutate(offense=fct_recode(offense,assault.weap="ASSAULT W/DANGEROUS WEAPON")) %>%
  ggplot(aes(start_date,hms::as_hms(start_date) )) + 
  geom_quasirandom(bandwidth = .00001,alpha=.2)  + 
...
Curious question - is the seasonal variation related to ACTUAL temperatures? Daylight?
...
df_wd=weather_history("Washington",
                    start = "2022-01-01",
                    end = "2022-12-31",
...
df_wd2=df_wd %>%
    separate(daily_sunset,c(NA,"sunset"),"T") %>%
  mutate(sunset=parse_time(sunset))

```
Let's look at temporal density while including sunset time.
...
crime_v2 |> 
  ...
  #geom_jitter(alpha=.2)+
  scale_x_datetime(date_breaks="1 month", date_labels="%b")+
  geom_line(data=df_wd2,aes(as_datetime(date),sunset),color="red")
