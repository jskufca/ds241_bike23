...
df2 %>% filter(month(t)==9) %>%
  ...
  ggplot(aes(hms::as_hms(t),riders,color=as.factor(epi))) +
  geom_line(alpha=.7) + 
>>>>>>> fa4f219d3c88502f011f8b3c6aabd03240c63523
...
df4 %>% 
  ...
  ggplot(aes(x=dur,y=as.factor(hour(started_at)))) +
  geom_density_ridges()
...
Is the wavy behavior in the morning an anomoly of small sample size?

```{r}
df4 %>% 
  filter(dur<60*30,dur>0) %>%
  #filter(day(started_at)<10) %>%
...
df4 %>% 
  ...
  #filter(day(started_at)==18) %>%
  #slice_sample(n=10000) %>%
  filter(hour(started_at)<5) %>%
  ggplot(aes(hms::as_hms(started_at),dur)) +
  geom_point(alpha=.1) 
...
df4 %>% 
  ...
  #filter(day(started_at)==18) %>%
  #slice_sample(n=10000) %>%
  filter(hour(started_at)<5,hour(started_at)>1) %>%
  ggplot(aes(hms::as_hms(started_at),dur)) +
